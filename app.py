...
def detect_language(text):
    resp = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": f"Detect the language of this text. Reply only the language name:\n{text}"
        }],
        temperature=0
    )
...
def translate(text, lang):
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": f"Translate this medical advice into natural {target}. Only return the translated text:\n{text}"
        }],
        temperature=0
    )
...
def chat():
    ...
    if not greeting and not has_context:
        if not is_medical(msg):
            fallback = "I am a medical assistant and can only help with health-related questions."

            code_map = {"hindi": "hi", "telugu": "te"}
            lang_code = code_map.get(lang, "en")

            fallback = translate(fallback, lang_code)
...
"""
    ...
    if selected_lang != "auto":
        reply = translate(reply, selected_lang)
